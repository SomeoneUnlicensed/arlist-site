...
const Status = () => {
  ...
  const componentDays = data.components.map((component: any) => historyForComponent(component.id, allIncidents, data.maintenance, settings.historyDays))
  const uptime = componentDays.length ? componentDays.reduce((sum: number, days: any[]) => sum + days.filter((day) => !day.impact).length / days.length * 100, 0) / componentDays.length : 100
  ...
  return <div className="status-page min-h-screen bg-[#f3f4ed] text-[#161815]">
    ...
    <main className="mx-auto max-w-[1120px] px-5 pb-24 pt-10 sm:px-8 sm:pt-14">
      ...
      <section className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">{[
        ['Работают сейчас', `${data.components.filter((item: any) => item.status === 'OPERATIONAL').length} / ${data.components.length}`],
        [`Доступность за ${settings.historyDays} дн.`, `${uptime.toFixed(uptime === 100 ? 0 : 2)}%`],
        ['Инциденты', String(allIncidents.length)],
        ['Обновление', `каждые ${settings.refreshSeconds} сек.`],
      ].map(([label, value]) => <div key={label} className="rounded-[18px] border border-black/10 bg-white/45 p-4"><p className="font-mono text-[9px] uppercase tracking-[.12em] text-black/35">{label}</p><p className="mt-2 text-lg font-semibold tracking-[-.03em]">{value}</p></div>)}</section>
